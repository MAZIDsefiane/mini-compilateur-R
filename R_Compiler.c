...
int testIdFunction(char * nom) {
    ...
    if(exist == -1)
    {
        ...
        strcpy(TableErreur[TableErreurCpt++],"Error: Function not declared");
        exit(0);
        return 0;
    }
    return exist;
...
void Test_Symbole(char* cl){
    ...
    sprintf(TableTest[TableTestCpt++], "#line %d : This token was read : %s  ", SYM_COUR.CODE);
...
void INST()
{
    ...
    else if(strcmp(SYM_COUR.CODE, "RETOUR_TOKEN") == 0) {
    }
    ...
    if(strcmp(SYM_COUR.CODE, "AFF_TOKEN") == 0) {
        ...
        if(index == -1) {
          //  printf("TOKEN %s NOT DEFINED\n", tempVarID);
            sprintf(TableErreur[TableErreurCpt++], "#line %d : TOKEN %s NOT DEFINED",Line, tempVarID);
        }
...
void parameter_list_def() {
    while(!strcmp(SYM_COUR.CODE, "ID_TOKEN")) {
        int cpt = TAB_IDFS2[comptTabFunct-1].nbparam++;
        strcpy(TAB_IDFS2[comptTabFunct-1].param[cpt],SYM_COUR.NOM);
        if(!strcmp(SYM_COUR.CODE, "VIR_TOKEN")) {
            Sym_Suiv();
            if(!strcmp(SYM_COUR.CODE, "PF_TOKEN")) {
                //printf("UNEXPECTED %s TOKEN\n", SYM_COUR.CODE);
                sprintf(TableErreur[TableErreurCpt++], "#line %d : NEXPECTED %s",Line, SYM_COUR.CODE );
                break;
            }
        }
        else {
...
{       int cptParam = 0;
    while(!strcmp(SYM_COUR.CODE, "ID_TOKEN") || !strcmp(SYM_COUR.CODE, "NUM_TOKEN") || !strcmp(SYM_COUR.CODE, "c_token")
        ...
        if(!strcmp(SYM_COUR.CODE, "c_token")) {
            vect();
                ...
                if(testVar(SYM_COUR.NOM)==0){
                //printf("var non declare \n");
                strcpy(TableErreur[TableErreurCpt++],"Error : Variable is not declared");
                   }
    ...
    if(cptParam != TAB_IDFS2[comptTabFunct-1].nbparam){
        ...
        strcpy(TableErreur[TableErreurCpt++],"Error : Parameters numbers is incorrect");
    }
}
